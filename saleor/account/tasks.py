...
def finish_creating_user(user_pk, redirect_url, channel_slug, context_data):
    ...
    logger.warning(f"[EMAIL DEBUG] finish_creating_user: user={user.email}, enable_confirmation={site.settings.enable_account_confirmation_by_email}, channel_slug={channel_slug}")
    
    if site.settings.enable_account_confirmation_by_email:
        # Notifications will be deprecated in the future
        token = token_generator.make_token(user)
        logger.warning(f"[EMAIL DEBUG] Calling send_account_confirmation for user {user.email}")
        ...
        logger.warning(f"[EMAIL DEBUG] send_account_confirmation completed for user {user.email}")
