...
import PageListPage from "@dashboard/modeling/components/PageListPage/PageListPage";
import { pageCreateUrl } from "@dashboard/modeling/urls";
import { articleListUrl, ArticleListUrlDialog, ArticleListUrlQueryParams } from "../../urls";
import { getFilterOpts, getFilterQueryParam, storageUtils } from "./filters";
import { getSortQueryVariables } from "./sort";

interface ArticleListProps {
  params: ArticleListUrlQueryParams;
}

const ArticleList = ({ params }: ArticleListProps) => {
  ...
  // Use pageTypes filter from params if provided, otherwise show all pages
  // Users can filter by page type using the filter UI

  usePaginationReset(articleListUrl, params, settings.rowNumber);
  ...
  const {
    ...
    getUrl: articleListUrl,
    storageUtils,
  });
  const paginationState = createPaginationState(settings.rowNumber, params);

  // Merge filters including pageTypes if provided
  const mergedFilter = useMemo(() => {
    ...
    // Add pageTypes filter if provided in params
    if (params.pageTypes && params.pageTypes.length > 0) {
      return {
        ...baseFilter,
    ...
    return baseFilter;
  }, [filter, params.query, params.pageTypes]);
  ...
  const newQueryVariables = useMemo(
    ...
    [params, settings.rowNumber, mergedFilter],
  );
  ...
  const [openModal, closeModal] = createDialogActionHandlers<
    ArticleListUrlDialog,
    ArticleListUrlQueryParams
  >(navigate, articleListUrl, params);
      ...
      if (data.pageBulkDelete?.errors.length === 0) {
        ...
        notify({
          ...
          text: intl.formatMessage({
            id: "vwA9Fq",
            defaultMessage: "Selected articles were deleted.",
  ...
  const handlePublish = async (selectedRowIds: string[]) => {
    ...
    notify({
      ...
      text: intl.formatMessage({
        id: "AUaL7R",
        defaultMessage: "Selected articles were published.",
        description: "notification",
      }),
    });
  };
  ...
  const handleUnpublish = async (selectedRowIds: string[]) => {
    ...
    notify({
      ...
      text: intl.formatMessage({
        id: "bnMF4j",
        defaultMessage: "Selected articles were unpublished.",
        description: "notification",
      }),
    });
  };

  const handleSort = createSortHandler(navigate, articleListUrl, params);
  const {
  ...
  return (
    <PaginatorContext.Provider value={paginationValues}>
      ...
      >
        <FormattedMessage
          id="8y4+0a"
          defaultMessage="{counter,plural,one{Are you sure you want to publish this article?} other{Are you sure you want to publish {displayQuantity} articles?}}"
      ...
      >
        <FormattedMessage
          id="8LWaFr"
          defaultMessage="{counter,plural,one{Are you sure you want to unpublish this article?} other{Are you sure you want to unpublish {displayQuantity} articles?}}"
          description="dialog content"
          values={{
            counter: selectedRowIds.length,
            displayQuantity: <strong>{selectedRowIds.length}</strong>,
      ...
      >
        <FormattedMessage
          id="8a4uf/"
          defaultMessage="{counter,plural,one{Are you sure you want to delete this article?} other{Are you sure you want to delete {displayQuantity} articles?}}"
          description="dialog content"
          values={{
            counter: selectedRowIds.length,
            displayQuantity: <strong>{selectedRowIds.length}</strong>,
